...
<div class="cron-tool">
    ...
    <h3>Use in JobRunr</h3>
    <div class="tabs">
        <button class="tab-btn active" onclick="showTab('jobrunr')">JobRunr</button>
        <button class="tab-btn" onclick="showTab('spring')">Spring</button>
        <button class="tab-btn" onclick="showTab('quartz')">Quartz</button>
    </div>
    <div id="code-jobrunr" class="code-examples">
<pre><span class="comment">// Using cron expression</span>
BackgroundJob.scheduleRecurrently(<span class="string">"my-job"</span>, <span class="string">"<span id="code-cron-1">0 9 * * *</span>"</span>,
    () -> myService.doWork());

<span class="comment">// Or use the Cron helper class</span>
BackgroundJob.scheduleRecurrently(<span class="string">"my-job"</span>, Cron.daily(<span id="code-hour">9</span>, <span id="code-minute">0</span>),
    () -> myService.doWork());</pre>
    </div>
    <div id="code-spring" class="code-examples" style="display:none;">
<pre><span class="keyword">@Scheduled</span>(cron = <span class="string">"<span id="code-cron-2">0 0 9 * * *</span>"</span>)
<span class="keyword">public void</span> scheduledTask() {
    myService.doWork();
}

<span class="comment">// Note: Spring uses 6 fields (includes seconds)</span></pre>
    </div>
    <div id="code-quartz" class="code-examples" style="display:none;">
<pre>CronTrigger trigger = TriggerBuilder.newTrigger()
    .withIdentity(<span class="string">"myTrigger"</span>)
    .withSchedule(CronScheduleBuilder.cronSchedule(<span class="string">"<span id="code-cron-3">0 0 9 * * ?</span>"</span>))
    .build();

<span class="comment">// Note: Quartz uses 6-7 fields and ? for day fields</span></pre>
    </div>
...
function showTab(tab) {
    ...
    document.querySelectorAll('.code-examples').forEach(e => e.style.display = 'none');
