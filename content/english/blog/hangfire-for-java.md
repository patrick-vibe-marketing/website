...
```java
BackgroundJob.enqueue<CarbonAware>(() -> generateReport());
```
...
If you're migrating a .NET application to Java and it uses Hangfire, here's the approach:

1. **Add JobRunr** to your Java project
2. **Map your job classes** to Java equivalents
3. **Convert the lambda syntax** (it's nearly identical)
4. **Configure storage** (same database often works)
5. **Run both** during migration if needed

The concepts map directly. You won't need to rethink your architecture.

## Pro Features Comparison

Both Hangfire and JobRunr have Pro versions with enterprise features.

| Feature | Hangfire Pro | JobRunr Pro |
|---------|--------------|-------------|
| Batches | ✓ | ✓ |
| Continuations | ✓ | ✓ |
| Redis Storage | ✓ | ✓ (OSS) |
...
JobRunr Pro includes some features that aren't available in Hangfire Pro, like SSO for the dashboard and multicluster monitoring.
